...
w = m.Wafer('NAN45_Jarray_quads_v2','DXF/',7000,7000,padding=0,waferDiameter=24000,sawWidth=200,#m.sawL_ws['8A'],
    ...
    def __init__(self,wafer,chipID,layer):
        ...
        for q in range(5):    
            ...
            for xy in [(250+q*spacing,100),(-250+(-4+q)*spacing,-400)]:
                ...
                self.add(dxf.text(str(np.round(fingerw,2)),sq1.getLastPos(labelPos),height=60,layer='MARKERS'))
                #fingerw=5
                ...
                Strip_straight(self, sq1, s_qubit/2-joverlap/2+r_qubit,w=leadw)
                CPW_straight(self, sq1, joverlap, w=fingerw,s=leadw/2-fingerw/2)
                CPW_taper(self, sq1, taper_l,w0=fingerw,s0=leadw/2-fingerw/2,w1=leadw,s1=0)
                sq1.shiftPos(-taper_l-joverlap/2,angle=180)
                self.add(dxf.rectangle(sq1.getPos((0,-fingerw/2)), finger2w, (joverlap2-leadw/2-fingerw)/2, valign=const.BOTTOM,halign=const.CENTER,rotation=sq1.direction,bgcolor=self.bg('SECONDLAYER'),layer='SECONDLAYER'))
                self.add(dxf.rectangle(sq1.start, finger2w+2*cdbias, fingerw, valign=const.MIDDLE,halign=const.CENTER,rotation=sq1.direction,bgcolor=self.bg('SECONDLAYER'),layer='SECONDLAYER'))
                self.add(dxf.rectangle(sq1.getPos((0,fingerw/2)), finger2w, (joverlap2-leadw/2-fingerw)/2, valign=const.TOP,halign=const.CENTER,rotation=sq1.direction,bgcolor=self.bg('SECONDLAYER'),layer='SECONDLAYER'))
                self.add(dxf.rectangle(sq1.getPos((-finger2w/2,joverlap2/2-leadw/4)), s_qubit/2+finger2w/2, leadw, rotation=sq1.direction,bgcolor=self.bg('SECONDLAYER'),layer='SECONDLAYER'),sq1,s_qubit/2)
            ...
            #upper row (manhattan square)
            s2=m.Structure(self,self.centered((250+q*spacing,1000)),direction=90)
            fingerw=np.round(0.4+0.2*q,2)
            self.add(dxf.text(str(np.round(fingerw,2)),s2.getLastPos(labelPos),height=60,layer='MARKERS'))
            overlap_junction_bilayer(self,s2,fingerw=fingerw,finger2w=fingerw,leadw=5.0,overlap=12,offset=5)
            s2=m.Structure(self,self.centered((-250-q*spacing,1000)),direction=90)
            fingerw=np.round(0.5+0.5*q,2)
            self.add(dxf.text(str(np.round(fingerw,2)),s2.getLastPos(labelPos),height=60,layer='MARKERS'))
            overlap_junction_bilayer(self,s2,fingerw=fingerw,finger2w=fingerw,leadw=5.0,overlap=12,offset=6)
            
            #bottom row (normal)
            fingerw=.4+0.2*q
            s2=m.Structure(self,self.centered((250+q*spacing,-1000)),direction=90)
            self.add(dxf.text(str(np.round(fingerw,2)),s2.getLastPos(labelPos),height=60,layer='MARKERS'))
            overlap_junction_bilayer(self,s2,fingerw=fingerw,finger2w=0.7,leadw=5.0,overlap=6)
            s2=m.Structure(self,self.centered((-250-q*spacing,-1000)),direction=90)
            fingerw=.5+0.5*q
            self.add(dxf.text(str(np.round(fingerw,2)),s2.getLastPos(labelPos),height=60,layer='MARKERS'))
            overlap_junction_bilayer(self,s2,fingerw=fingerw,finger2w=0.7,leadw=5.0,overlap=6)
...
class QSearchChip6(m.Chip7mm):
    ...
    def __init__(self,wafer,chipID,layer,
                 ...
                 coupler_length=180,
                 fingerws =   [0.8,0.95,1.1,1.25,1.4,1.6],  # first layer
                 finger2ws =   [0.8,0.8,0.85,0.9,0.95,0.95],# second layer
...
class InverseQSearchChip6(m.Chip7mm):
    ...
    def __init__(self,wafer,chipID,layer,
                 ...
                 seps =       [7+4]*6,#distance to cpw
                 coupler_length=180,
                 fingerws =   [0.8,0.8,0.85,0.9,0.95,0.95],  #second layer
                 finger2ws =   [0.8,0.95,1.1,1.25,1.4,1.6], #first layer
                 joverlaps =  [4,4,4,4.5,5,5.5],
                 q_lengths = [282,262,242,222,202,182],
                 q_overlaps = [200,180,160,140,120,100],
                 indices =    [5,0,3,4,1,2],#[2,1,4,3,0,5],
                 cd_bias = 0,
                 res_spacing=1300):
        m.Chip7mm.__init__(self,wafer,chipID,layer,defaults={'w':10, 's':6, 'radius':300,'r_out':10,'r_ins':10,'curve_pts':30})
...
class ResonatorChip6(m.Chip7mm):
    def __init__(self,wafer,chipID,layer,
        ...
        for i in range(6):
            ...
            JellyfishResonator(self,s1,500,412,None,r_ind=4,w_ind=3,w_cap=40,s_cap=20,maxWidth=L_ws[i]/2.,nTurns=19,ialign=const.TOP,layer='XOR2')
...
#doMirrored(MarkerCross, w, (16000,16000),linewidth=2, layer='MARKERS')
#doMirrored(MarkerCross, w, (16500,16500),linewidth=4, layer='MARKERS')
#doMirrored(MarkerCross, w, (17000,17000),linewidth=8, layer='MARKERS')
